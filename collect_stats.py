...
def main():
    ...
    for repo in REPOS:
        ...
        search = gh(f"/search/issues?q=repo:{OWNER}/{repo}+is:pr+is:open&per_page=1")
        pr_count = search["total_count"] if search else 0
        issue_count = max(0, info["open_issues_count"] - pr_count)
        ...
        badge(f"badges/prs/{repo}.json",    "open PRs", pr_count,                 "blue")
